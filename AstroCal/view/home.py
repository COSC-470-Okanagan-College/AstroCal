...
if MODE == 'GUI':
    ...
    class HBoxLayoutExample(App):
        ...
        middlelayout = BoxLayout(size_hint=(
            1, None), height=400, center_x=True)
        ...
        # Add display features to middle primary display window (defaults to day view)
            dayInfo = AnchorLayout(size_hint=(
                1, None), height=300, anchor_x='right', anchor_y='center')
            dayImage = BoxLayout(orientation='vertical',
                                 size_hint=(1, None), height=300)
            wimg = Image(source="view/assets/moon_phases/full_moon.png",
                         allow_stretch=True)
        ...
        def getSunRise(self):
            now = datetime.now()
            sun_rise = control.getRiseSet(
                now.year, now.month, now.day, 'SUN', 'RISE')
        ...
        def getSunSet(self):
            now = datetime.now()
            sun_rise = control.getRiseSet(
                now.year, now.month, now.day, 'SUN', 'SET')
        ...
        def getMoonSet(self):
            now = datetime.now()
            moon_set = control.getRiseSet(
                now.year, now.month, now.day, 'MOON', 'SET')
        ...
        def utc_hack(self, date_tup):
            ...
            timeUTC = datetime(
                date_tup[0], date_tup[1], date_tup[2], date_tup[3], date_tup[4], 0, 0, pytz.UTC)
